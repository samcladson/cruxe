 ...
 *
 * The generator persists the built grid so that every player solves the same
 * layout and the server holds the answer key it verifies against.
 */
function hydrateStoredPuzzle(pd: any, row: any): Puzzle {
  const grid: GridCellType[][] = pd.grid.map((r: any[], rowIndex: number) =>
    r.map((c: any, colIndex: number) => ({
      row: rowIndex,
      col: colIndex,
      letter: c.letter,
...
export async function fetchPuzzleById(
  ...
  console.warn(
    `[puzzleService] Puzzle ${puzzleId} has no stored grid (legacy row); ` +
      "solving it will not be server-verifiable.",
  );
